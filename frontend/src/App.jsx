...
function starterSuggestions(question='') {
  const rules = [
    [['음식','간식','먹','요리'], ['내가 제일 좋아했던 건', '그 맛을 떠올리면', '우리 동네에는']],
    [['노래','음악','취미'], ['한때 정말 좋아했던 건', '시간 가는 줄 몰랐던 건', '지금도 기억나는 건']],
    [['여행','장면','계절'], ['그곳에서 가장 기억나는 건', '처음 도착했을 때', '다시 간다면 꼭']],
    [['학창','친구','웃음'], ['학교 다닐 때 우리는', '제일 많이 웃었던 건', '아직도 기억나는 친구는']],
    [['실패','결과','포기','도전','견뎌'], ['처음엔 정말 막막했지', '내가 다시 움직인 계기는', '돌이켜 보면 그 경험은']],
    [['관계','사람','이별','마음이 멀어'], ['그때 가장 아쉬웠던 건', '먼저 마음을 열게 된 건', '지나고 나서 알게 된 건']],
    [['선택','갈림길','결정','진로'], ['내가 가장 중요하게 본 건', '결정을 내리기 전에는', '지금 다시 선택해도']],
    [['돈','월급','형편','경제'], ['당시에는 넉넉하지 않았지만', '가장 먼저 줄였던 건', '그래도 지키고 싶었던 건']],
    [['지쳤','불안','혼자','걱정'], ['그럴 때 나는 먼저', '마음을 조금 놓게 된 건', '하루를 버티게 해준 건']],
  ]
  const match = rules.find(([keywords]) => keywords.some((keyword) => question.includes(keyword)))
  return match?.[1] || ['가장 먼저 떠오르는 건', '그때의 나는', '지금 생각해 보면']
}
...
export default function App() {
  ...
  const [library,setLibrary] = useState(()=>{
    try { return JSON.parse(localStorage.getItem('seuljjeock-library')) || seeds }
    catch { return seeds }
  })
  ...
  const timer = useRef()
  useEffect(()=>{ localStorage.setItem('seuljjeock-library',JSON.stringify(library)) },[library])
  ...
  async function transform() {
    ...
    try { const r = await post('/api/questions/transform',{worry,emotion,mode}); setQuestionOptions(r.questions||[r.question]); setQuestion(r.question); if(r.source==='local') setNotice('자녀 질문을 네 개의 일상 질문 사이에 섞었어요.') }
  ...
  async function polish() {
    ...
    setLoading(true)
    try { const r = await post('/api/answers/polish',{answer,question}); setPolished(r.polished); if(r.source==='local') setNotice('AI 연결이 없어 질문 맞춤형 로컬 다듬기를 사용했어요.') }
    catch { setPolished('나도 그 순간에는 앞이 보이지 않았단다. 그래도 하루씩 견디며 지나고 보니, 그때의 실패가 다음 길을 알아보는 힘이 되어 주었어.') }
    finally { setLoading(false) }
  }
  function save() {
    setLibrary([{id:Date.now(),emotion,date:formatDate(new Date()),author:'아빠',question,raw:answer,polished,thanks:0,moved:0},...library])
  ...
  return <div className="app">
    ...
    {screen==='child' && <main className="page child">
      ...
      <section className="paper">
        ...
        <div className="emotions">{emotions.map(([name,icon])=><button key={name} className={emotion===name?'selected':''} onClick={()=>setEmotion(name)}><i className="emotion-icon">{icon}</i><span>{name}</span>{emotion===name&&<em><Icon type="check"/></em>}</button>)}</div>
    ...
    {screen==='parent' && <main className="page parent">
      ...
      <section className="question question-feed"><div><b>오늘 도착한 이야기 질문</b><em>{questionOptions.length||1}개의 질문</em></div><p className="feed-intro">가볍게 떠오르는 질문부터 골라보세요. 짧은 추억도, 오래된 경험도 모두 좋은 답이에요.</p><div className="question-options">{(questionOptions.length?questionOptions:[question||fallback]).map((item,index)=><button key={item} className={question===item?'selected':''} onClick={()=>setQuestion(item)}><span>{String(index+1).padStart(2,'0')}</span><b>{item}</b>{question===item&&<i>선택됨</i>}</button>)}</div><small>✦ 가족이 보낸 질문도 일상 질문 사이에 자연스럽게 섞여 있어요.</small></section>
      <section className="paper answer"><div className="answer-head"><div><h2>내 경험 들려주기</h2><p>말하거나, 직접 적어도 좋아요.</p></div><span>약 1분</span></div>
        ...
        <div className="chips"><span>시작이 어렵다면</span>{starterSuggestions(question).map(x=><button key={x} onClick={()=>setAnswer(answer+(answer?' ':'')+x+'…')}>+ {x}</button>)}</div>
    ...
    {screen==='library' && <main className="page library">
      ...
      <div className="shelf">날짜별 이야기 <i/></div>
      <div className="wisdom-list">{library.map((x,i)=><div className="dated-response" key={x.id}>
        {(i===0||library[i-1].date!==x.date)&&<div className="date-heading"><div><b>{x.date}</b><span>{library.filter(item=>item.date===x.date).length}개의 응답</span></div><i/></div>}
        <article className={'wisdom '+(i===0?'latest':'')}><div className="wisdom-top"><div><span>{x.emotion}의 날</span>{i===0&&<em>NEW</em>}</div><time>{x.date}</time></div><small>{x.question}</small><blockquote>“{x.polished}”</blockquote><div className="author"><b>{x.author[0]}</b><div><strong>{x.author}의 이야기</strong><span>우리 가족에게</span></div></div><div className="wisdom-foot"><div><button onClick={()=>react(x.id,'thanks')}>♡ 고마워요 {x.thanks||''}</button><button onClick={()=>react(x.id,'moved')}>✦ 감동이에요 {x.moved||''}</button></div><button onClick={()=>setOpenRaw(openRaw===x.id?null:x.id)}>{openRaw===x.id?'원문 닫기':'처음 건넨 말 보기'}</button></div>{openRaw===x.id&&<div className="raw"><b>처음 건넨 말</b><p>{x.raw}</p></div>}</article>
      </div>)}</div><button className="new" onClick={()=>go('child')}>♡ 새로운 마음 보내기</button>
    </main>}
