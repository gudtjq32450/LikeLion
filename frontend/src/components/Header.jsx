...
export default function Header({
  token, family, user, userRole, screen, go, pendingCount, apiStatus, aiReady, onLogout
}) {
  return (
    <header>
      ...
      <div className="family">
        ...
        {user && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span>
              {family?.name || '가족'} <b>{user.name}</b>
            </span>
            <button onClick={onLogout} title="로그아웃" style={{ border: 0, background: 'none', color: 'var(--muted)', cursor: 'pointer' }}>
              <Icon type="logout" />
            </button>
          </div>
        )}
